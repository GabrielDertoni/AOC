...
while not todos.empty?
  ...
  for curr_scanner in scanners.keys
    ...
    if inter.size >= 12
      all_beacons |= transf
      scanner_positions[curr_scanner] = Int64.cast(scanner_b_to_a[3, 0...3]).to_a
      todos.delete curr_scanner
    end
  end
end

puts scanner_positions.values.product(scanner_positions.values).lazy.map { |i, j| dist(i, j) }.max
